...
Item {
  ...
  // Third-party bars are instantiated asynchronously and receive host
  // properties from shell.configureBar() in Loader.onLoaded. They cannot be
  // `required`: QML validates required properties before onLoaded can inject
  // them. Defaults keep the component constructible during that short gap.
  ...
  // Omarchy's stock bar paints one edge-to-edge slab. Osuki themes opt into
  // a true pill surface through custom [bar] tokens in shell.toml. Keeping
  // this behind a theme token makes the cloned bar behave exactly like the
  // stock bar for every other theme.
  function themeBool(key, fallback) {
    var value = Color.shellValues[key]
  ...
  function themeNumber(key, fallback) {
    var value = Number(Color.shellValues[key])
    return isFinite(value) && value >= 0 ? value : fallback
  }

  readonly property bool pillMode: themeBool("bar.pill", false)
  readonly property real pillEdgeMargin: themeNumber("bar.pill-edge-margin", 8)
  readonly property real pillCrossMargin: themeNumber("bar.pill-cross-margin", 3)
  readonly property real pillPadding: themeNumber("bar.pill-padding", 10)
  readonly property int pillBorderWidth: Math.round(themeNumber("bar.pill-border-width", 1))
  readonly property color pillBorder: Color.composed(
    "bar.pill-border",
    "bar.pill-border-alpha",
    root.barForeground,
    0.18)
  ...
  component BarPanel: PanelWindow {
    ...
    color: root.transparent || root.pillMode ? "transparent" : root.background
    ...
    Component {
      ...
      Item {
        ...
        PillModuleList {
          ...
          region: "left"
          anchors.left: parent.left
          anchors.leftMargin: root.pillEdgeMargin
          anchors.verticalCenter: parent.verticalCenter
        }
        ...
        PillModuleList {
          ...
          region: "right"
          anchors.right: parent.right
          anchors.rightMargin: root.pillEdgeMargin
          anchors.verticalCenter: parent.verticalCenter
        }
      }
    }
    ...
    Component {
      ...
      Item {
        ...
        PillModuleList {
          ...
          region: "left"
          anchors.top: parent.top
          anchors.topMargin: root.pillEdgeMargin
          anchors.horizontalCenter: parent.horizontalCenter
        }
        ...
        PillModuleList {
          ...
          region: "right"
          anchors.bottom: parent.bottom
          anchors.bottomMargin: root.pillEdgeMargin
          anchors.horizontalCenter: parent.horizontalCenter
        }
      }
    }
  ...
  Component { id: localTrayWidgetComponent; LocalWidgets.Tray {} }
  Component { id: localIndicatorsWidgetComponent; LocalWidgets.Indicators {} }
  ...
  component PillModuleList: BorderSurface {
    id: pillSection

    property var entries: []
    property string region: ""
    property bool centerSection: false
    visible: root.pillMode && entries.length > 0
    implicitWidth: modules.implicitWidth + root.pillPadding * 2
    implicitHeight: Math.max(modules.implicitHeight, root.barSize - root.pillCrossMargin * 2)
    width: implicitWidth
    height: implicitHeight
    color: root.transparent ? "transparent" : root.background
    borderSpec: root.transparent
      ? Border.none()
      : Border.flat(root.pillBorder, root.pillBorderWidth)
    radius: Math.min(width, height) / 2

    CenterGestureArea {
      visible: pillSection.centerSection
      anchors.fill: parent
    }

    HoverHandler {
      enabled: pillSection.centerSection
      onHoveredChanged: if (pillSection.centerSection) root.setCenterSectionHovered(hovered)
    }

    ModuleList {
      id: modules
      entries: pillSection.entries
      region: pillSection.region
      anchors.centerIn: parent
    }
  }
  ...
  component ModuleSlot: Item {
    ...
    readonly property var registryComponent: {
      ...
      // These two widgets contain Osuki-specific collapsing geometry. The
      // global registry points at Omarchy's packaged copies even when the full
      // bar is cloned, so select the local components explicitly; all other
      // widgets continue to track the system implementation.
      if (registryName === "omarchy.tray") return localTrayWidgetComponent
      if (registryName === "omarchy.indicators") return localIndicatorsWidgetComponent
    ...
    Rectangle {
      ...
      scale: slot.panelOpen && !slot.dragSource ? 1 : 0.35
      color: Color.accent
      radius: Math.min(width, height) / 2
