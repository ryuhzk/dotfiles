...
This user-owned clone preserves the stock `omarchy.bar` behavior unless the
active theme sets `bar.pill = true` in `shell.toml`. Pill themes can also set
`bar.pill-edge-margin` and `bar.pill-cross-margin`; the surface then uses a
true half-height radius independently of Hyprland window rounding.
