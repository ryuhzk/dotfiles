...
BarWidget {
  ...
  // Compact enough to feel attached to the pill instead of exposing a long,
  // empty track while the drawer catches up with the cursor.
    ...
    Item {
      ...
      Item {
        ...
        BarIconButton {
          ...
          y: 0
          text: "\uf053"
          textRotation: root.expanded ? 270 : 90
          Behavior on textRotation {
            NumberAnimation { duration: root.animationDuration; easing.type: Easing.OutCubic }
          }
          onPressed: function(button) {
